...
def _log_shape(stage, labels):
    imf = labels.get("im_file", "<no-file>")
    img = labels.get("img", None)
    if img is None:
        LOGGER.info(f"[{stage}] im_file={imf}  img=None")
        return
    if isinstance(img, torch.Tensor):
        shape = tuple(img.shape)  # C,H,W
        LOGGER.info(f"[{stage}] im_file={imf}  torch img shape={shape}, dtype={img.dtype}")
    else:
        # numpy
        LOGGER.info(f"[{stage}] im_file={imf}  np img shape={img.shape}, dtype={img.dtype}")
    if hasattr(img, "shape") and len(img.shape) == 3:
        c = img.shape[2] if not isinstance(img, torch.Tensor) else img.shape[0]
        assert c in (1, 3, 4, 5, 6), f"[{stage}] Unexpected channels={c} for {imf}"
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        def _hsv_jitter_3ch(bgr):
            h, s, v = cv2.split(cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV))
            im_hsv = cv2.merge((cv2.LUT(h, lut_hue), cv2.LUT(s, lut_sat), cv2.LUT(v, lut_val)))
        ...
        if c == 3:
            labels["img"] = _hsv_jitter_3ch(img_uint8)
            return labels

        if c == 6:
            left = img_uint8[..., :3]
            right = img_uint8[..., 3:6]
            img_uint8[..., :3] = _hsv_jitter_3ch(left)
            img_uint8[..., 3:6] = _hsv_jitter_3ch(right)
            labels["img"] = img_uint8
            return labels

        last3 = img_uint8[..., -3:]
        img_uint8[..., -3:] = _hsv_jitter_3ch(last3)
        labels["img"] = img_uint8
        return labels
