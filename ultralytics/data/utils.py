...
def imread_keep_channels(path: str) -> np.ndarray:
...
def verify_image(args):
    ...
    nf, nc, msg = 0, 0, ""
    try:
        suffix = Path(im_file).suffix.lower()

        if suffix in {".tif", ".tiff"}:
            arr = imread_keep_channels(im_file)
            if arr is None:
                raise ValueError("image is None")
            h, w = arr.shape[:2]
            assert (h > 9) & (w > 9), f"image size {(h, w)} <10 pixels"
            nf = 1
        else:
            im = Image.open(im_file)
            im.verify()  # PIL verify
            shape_wh = exif_size(im)  # (w, h)
...
def verify_image_label(args):
    ...
    try:
        ...
        suffix = Path(im_file).suffix.lower()
        if suffix in {".tif", ".tiff"}:
            arr = imread_keep_channels(im_file)  # HxWxC 或 HxW->HxWx1
            if arr is None:
                raise ValueError("image is None")
            h, w = arr.shape[:2]
            assert (h > 9) & (w > 9), f"image size {(h, w)} <10 pixels"
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                if keypoint:
                    exp_cols = 5 + nkpt * ndim
                    assert lb.shape[1] == exp_cols, f"labels require {exp_cols} columns each"
        ...
        if keypoint:
            keypoints = lb[:, 5:].reshape(-1, nkpt, ndim) if lb.size else np.zeros((0, nkpt, ndim), dtype=np.float32)
            if ndim == 2 and keypoints.size:
