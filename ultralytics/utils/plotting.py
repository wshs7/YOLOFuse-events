...
def _to_vis3(img_chw):
    """
    把 CHW 的图像（C 可以是 1/2/3/4/6/…）转换成 HWC 的 3 通道，仅用于可视化。
    规则：
      - C==1：重复到3通道
      - C==2：在后面补一个全零通道 -> [C0, C1, 0]
      - C>=3：取前三个通道
    """
    if isinstance(img_chw, torch.Tensor):
        img_chw = img_chw.detach().cpu().numpy()
    assert img_chw.ndim == 3, f"expect CHW, got shape={img_chw.shape}"
    c, h, w = img_chw.shape
    img_hwc = img_chw.transpose(1, 2, 0)  # HWC

    if c == 1:
        return np.repeat(img_hwc, 3, axis=2)
    elif c == 2:
        zero = np.zeros_like(img_hwc[..., :1])
        return np.concatenate([img_hwc, zero], axis=2)
    elif c== 5:
        return img_hwc[...,2:5]
    else:
        return img_hwc[..., :3]
...
def plot_images(
    ...
    for i in range(bs):
        ...
        mosaic[y : y + h, x : x + w, :] = _to_vis3(images[i])
