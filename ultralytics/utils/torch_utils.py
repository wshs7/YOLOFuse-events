...
def _infer_in_ch(model, default=3):
    # 从 model.args 取 ch；兼容 dict 或对象；取不到就用 default
    a = getattr(model, 'args', None)
    if isinstance(a, dict):
        return int(a.get('ch', default))
    if hasattr(a, 'ch'):
        return int(a.ch)
    return int(default)
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            in_ch = _infer_in_ch(model, default=2)  # ★ 关键：从模型读输入通道
            im = torch.empty((1, in_ch, stride, stride), device=p.device)
            ...
            # 用真实 imgsz
            in_ch = _infer_in_ch(model, default=3)
            im = torch.empty((1, in_ch, *imgsz), device=p.device)
            flops = thop.profile(deepcopy(model), inputs=[im], verbose=False)[0] / 1e9 * 2
            if not flops:
                flops = get_flops_with_torch_profiler(model, imgsz)
            return flops
...
def get_flops_with_torch_profiler(model, imgsz=640):
    """Compute model FLOPs (thop package alternative)."""
    if not TORCH_2_0:
        return 0.0
    model = de_parallel(model)
    p = next(model.parameters())
    ...
    try:
        ...
        in_ch = _infer_in_ch(model, default=5)  # ★ 同上
        im = torch.empty((1, in_ch, stride, stride), device=p.device)
        with torch.profiler.profile(with_flops=True) as prof:
            model(im)
        flops = sum(x.flops for x in prof.key_averages()) / 1e9
