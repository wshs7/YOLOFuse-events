...
def _find_first_conv(module: nn.Module):
    for name, m in module.named_modules():
        if isinstance(m, nn.Conv2d):
            ...
            return m, parent, parts[-1]
        if hasattr(m, "conv") and isinstance(getattr(m, "conv"), nn.Conv2d):
            return getattr(m, "conv"), m, "conv"
    return None, None, None

def adapt_first_conv_to_2ch(root: nn.Module, mode: str = "avg"):

    conv, parent, attr = _find_first_conv(root)
    assert conv is not None, "Could not find first Conv2d"
    if conv.in_channels == 2:
        return
    assert conv.in_channels == 3, f"Expected in_channels=3, got {conv.in_channels}"

    w_old = conv.weight.data.clone()                     # [out, 3, k, k]
    b_old = conv.bias.data.clone() if conv.bias is not None else None
    ...
    if mode == "avg":
        c0, c1, c2 = w_old[:, 0:1], w_old[:, 1:2], w_old[:, 2:3]
        w_new = torch.cat([(c0 + c1) / 2.0, (c1 + c2) / 2.0], dim=1)
    elif mode == "slice":
        w_new = w_old[:, :2]
    else:
        raise ValueError("mode must be 'avg' or 'slice'")

    new_conv.weight.data.copy_(w_new)
    if b_old is not None:
        new_conv.bias.data.copy_(b_old)

    assert parent is not None and attr is not None
    setattr(parent, attr, new_conv)

def adapt_first_conv_to_5ch(root: nn.Module, mode: str = "avg"):

    conv, parent, attr = _find_first_conv(root)
    assert conv is not None, "Could not find first Conv2d"
    if conv.in_channels == 5:
        return

    Cin_old = conv.in_channels
    w_old = conv.weight.data.clone()  # [out, Cin_old, k, k]
    b_old = conv.bias.data.clone() if conv.bias is not None else None
    out_c, (k_h, k_w) = conv.out_channels, conv.kernel_size
    ...
    if Cin_old >= 3:
        ...
        if Cin_old == 1:
            w_base3 = w_old.repeat(1, 3, 1, 1)
        else:  # Cin_old == 2
            mean_12 = w_old.mean(dim=1, keepdim=True)  # [out,1,k,k]
            w_base3 = torch.cat([w_old, mean_12], dim=1)  # [out,3,k,k]

    if mode == "avg":
        mean3 = w_base3.mean(dim=1, keepdim=True)  # [out,1,k,k]
        w_extra2 = mean3.repeat(1, 2, 1, 1)        # [out,2,k,k]
    elif mode == "zero":
        w_extra2 = torch.zeros((out_c, 2, k_h, k_w), device=w_old.device, dtype=w_old.dtype)
    else:
        raise ValueError("mode must be 'avg' or 'zero'")

    w_new = torch.cat([w_base3, w_extra2], dim=1)  # [out,5,k,k]
    new_conv.weight.data.copy_(w_new)
    if b_old is not None:
        new_conv.bias.data.copy_(b_old)
...
class Model(nn.Module):
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        try:
            exp_ch = int(args.get("ch", getattr(getattr(self.trainer.model, "args", None), "ch", 3)))
        except Exception:
            exp_ch = 3

        if exp_ch == 2:
            try:
                adapt_first_conv_to_2ch(self.trainer.model, mode="avg")  # 从 3→2
                if hasattr(self.trainer.model, "args"):
                    self.trainer.model.args.ch = 2
                LOGGER.info("Adapted first conv to 2 input channels.")
            except Exception as e:
                LOGGER.warning(f"WARNING ⚠️ failed to adapt first conv to 2 channels: {e}")
