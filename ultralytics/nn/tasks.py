...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        #     c2 = (ch[f] // 2)
            ...
            in_ch = ch[f]                   # 来自 from 的输入通道数（这里是 5）
            out_id = args[0] if len(args) > 0 else 1
            split = args[1] if len(args) > 1 and args[1] is not None else in_ch // 2
            # 规范化参数，回填到 args，确保实例化时带上 split
            args = [out_id, split]
            # 计算本层输出通道
            if out_id == 1:
                c2 = split                 # 前半段：2
            else:
                c2 = in_ch - split         # 后半段：5-2=3
